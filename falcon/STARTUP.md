...
この形態は入れ物（コンテナ）の中で動く 1 本道です。ホストで直接 `python server.py` を
...
| `--reset-admin` | **この系統の入口（`./launch.sh`）には配線されていません。**この形は入れ物（コンテナ）の中で動く1本道で、入口はこの指定を受け取りません（`./launch.sh` に受け口 0 件）。管理者の合言葉は、画面に入ってから変えてください。最初の合言葉は下の「ログイン」の節にあります |
