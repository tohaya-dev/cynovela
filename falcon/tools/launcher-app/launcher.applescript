...
on askSettings(repo)
	...
	if b is "指定する" then
		set r2 to display dialog "ポート番号" default answer "8801" buttons {"キャンセル", "続ける"} default button "続ける" with title "Cynovela"
...
on run
	...
	repeat
		set egLine to runCore(repo, "engine")
		if egLine contains "ENGINE=ok" then exit repeat
		if egLine contains "ENGINE=not-running" then
			set AppleScript's text item delimiters to "NAME="
			set egName to text item 2 of egLine
			set AppleScript's text item delimiters to ""
			display dialog "コンテナの土台（" & egName & "）は入っていますが、まだ動いていません。" & return & "アプリを開くか、端末で起動してから、もう一度この印を押してください。" & return & "Podman の場合: podman machine start" buttons {"OK"} default button "OK" with icon caution with title "Cynovela"
			return
		end if
		set aBtn to button returned of (display dialog "この形はコンテナの上で動きます。この機械では Podman も Docker も見つかりませんでした。" & return & "・入れてから、もう一度この印を押してください。" & return & "　Podman: https://podman.io/    Docker: https://www.docker.com/" & return & "・すでに入っている場合や、別のものをお使いの場合は「場所を選ぶ」から、" & return & "　その実行ファイルを指してください。" buttons {"場所を選ぶ", "閉じる"} default button "閉じる" with icon stop with title "Cynovela")
		if aBtn is "閉じる" then return
		try
			set sel to POSIX path of (choose file with prompt "コンテナの土台の実行ファイルを選んでください")
		on error
			return
		end try
		runCore(repo, "engine set " & quoted form of sel)
	end repeat
