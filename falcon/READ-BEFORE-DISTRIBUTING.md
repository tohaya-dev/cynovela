...
**どちらも再起動して消えることはありません。**置き場が分かれているので、デモで試したあとに本番を使い始めても、デモの中身が本番に混ざることはありません。
...
同梱のデータベースと索引は、**配布物を作るときに、この配布物の中の `dummy-corpus/` から作っています**。作る側の作業用の資料や索引は入っていません。コンテナ版には、実際に入っているものを梱包の場で数えた内訳が `BUNDLED-DATA.md` として同梱されます。
