...
async def update_workspace(ws_id: str, request: Request):
    ...
    body = await parse_body_pydantic(request)
    conn = get_db()
    # connleak-fix-20260709: INSERT 例外 (重複 ID の UNIQUE/PK 違反等) で conn を開いたまま
    # 抜けると暗黙 BEGIN の書き込みトランザクションが残留し "database is locked" になるため、
    # try/finally で close を保証する。重複 ID は 409 で穏当に弾く。
    try:
        ws = conn.execute("SELECT * FROM workspaces WHERE id = ?", (ws_id,)).fetchone()
        if not ws:
            raise HTTPException(404, "Workspace not found")
        ...
        if "source_ids" in body:
            ...
            for sid in body["source_ids"]:
                try:
                    ...
                    raise HTTPException(409, f"重複したsource_idが指定されています: {sid}") from e
        if "user_ids" in body:
            ...
            for uid in body["user_ids"]:
                try:
                    ...
                    raise HTTPException(409, f"重複したuser_idが指定されています: {uid}") from e
        conn.commit()
    finally:
        conn.close()
