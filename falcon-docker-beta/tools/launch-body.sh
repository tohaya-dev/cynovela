...
_roots_py() {
    if [ -z "$ROOTS_PY" ]; then
        echo "エラー: 3.12 系の python が見つかりません。取り込み元のバックアップ (store/ingest-roots.json) を扱えません。" >&2
...
_print_model_next_steps() {
    ...
    echo ""
    echo "  どちらも、保存先の形は docs/SETUP-ACCELERATOR.md の手順に合わせてください。"
}
...
run_probe() {
    ...
    if [ -n "$ROOTS_PY" ]; then
        ...
        add_report "バックアップに使う python: ありません (3.12 系が見つかりません)"
        if [ -s "$INGEST_ROOTS_FILE" ]; then
            add_blocker "3.12 系の python が見つかりません。取り込み元のバックアップ (store/ingest-roots.json) を読めず、足したフォルダが読み込まれません。./Cynovela-start.command で「1) conda で作る」を選んでください。"
        fi
    fi
