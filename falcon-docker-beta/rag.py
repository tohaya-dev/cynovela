...
def _current_embedding_model_name() -> str:
    """いま使っている Embedding モデル名を返す唯一の読み手。

    LocalSentenceTransformerProvider / MLXEmbeddingProvider は属性 model_name、
    ...
    画面の設定欄と開発者パネルで食い違った (実測: 設定欄 bge-m3-mas-a4 /
    開発者パネル BAAI/bge-m3)。読み手をこの関数へ一本化する。
    """
    p = _embedding_provider
    name = getattr(p, "model_name", None) or getattr(p, "model", None)
    if name:
        return str(name)
    cls = type(p).__name__
    if "TFIDF" in cls:
...
def _current_embedding_identity() -> dict:
    ...
    #   (§9-4 の突き合わせは「インデックスの記録」と「相手が実際に積んでいるモデル」を比べるためのもの)。
    _model_name = _current_embedding_model_name()
    _rev = "unknown"
    try:
...
def _normalize_role_to_acl(role: str) -> str:
    """BLOCK B-1: ログインロールを ACL ロールにマップする。
    有効なロールは admin / viewer の 2 つだけ (core.constants.VALID_ROLES) で、
    ACL ロールはログインロールと同じ値のため、前後の余白を落として返すだけでよい。
    """
