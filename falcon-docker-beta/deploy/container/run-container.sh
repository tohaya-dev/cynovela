...
#  決めごとの出どころ: cynovela.yaml (server.port / container.name / container.image /
#  container.volume_prefix)。入口からは指定 (--from-entry / --hostport) だけを受ける。
#
# multi-ingest-roots-20260728: 取り込み元は控えファイル (store/ingest-roots.json) に登録した
#   複数の根を /app/ingest/<中の名前> へ読み取り専用 (:ro) で個別マウントする方式。
#   取り込み元は控え (store/ingest-roots.json) から決まる。
# ============================================================
set -euo pipefail
REPO="$(cd "$(dirname "$0")/../.." && pwd)"

# 入口を1本にするための門。直接叩かれたときは、何もせず入口を案内する。
...
#   入れ物の名前は入口 (launch.sh) と同じ1つの値であり、途中で読み替えない。
CONF_REPO="$REPO"
. "$REPO/tools/conf.sh"
...
# DD-CYN-0074 Q-1: モデルの置き場。入口 (tools/launch-body.sh) と同じ1行を読む。
...
# DD-CYN-0107 F-c: 控えの読み書きは動作要件 (3.12 系) を満たす python でのみ行う。
#   素の python3 (版の検査なし) へは倒れない。3.12 系が無く控えが在るときは黙って0件へ倒さず止める。
ROOTS_PY="$(conf_pick_py "$REPO" || true)"
if [ -z "$ROOTS_PY" ]; then
  if [ -s "$ROOTS_FILE" ]; then
    echo "エラー: 3.12 系の python が見つからないため、取り込み元の控え ($ROOTS_FILE) を読めません。" >&2
    echo "       ./Cynovela-start.command を一度押して「1) conda で作る」を選ぶと、この配布物の中に 3.12 の環境が作られます。" >&2
    exit 2
  fi
  echo "[roots] 3.12 系の python が見つかりません。取り込み元の控えは扱いません (同梱のダミー資料を根にします)"
fi
_roots() { [ -n "$ROOTS_PY" ] || return 1; "$ROOTS_PY" "$ROOTS_HELPER" --file "$ROOTS_FILE" "$@"; }

# 控えの roots 配列を PUT /api/settings 用の {"ingest.roots": "<JSON文字列>"} に整形する
# portable-roots-20260808 (DD-CYN-0066 F-2): 生の JSON を直に読むと、配布物の根からの
#   相対の書き方 ("@app/…") がそのまま画面の表示写像へ流れる。∴ 控えを読む側は
#   scripts/ingest_roots.py に一本化し、解いたホスト側の絶対パスだけを送る。
_roots_payload() {
  _roots list | "$ROOTS_PY" -c '
import json, sys
...
# bundled-config-20260731: モデル置き場が無いまま podman へ渡すと、bind 元不在で
#   'Error: statfs ... no such file or directory' という素の失敗になり、受け取り手には
#   何をすればよいか分からない。軽量版は store/models を同梱しないため必ずここに当たる。
#   先に自分で検出して、SETUP-ACCELERATOR.md の手順を名指しで案内する。
if [ ! -d "$MODEL_ROOT" ]; then
  echo "[models] 埋め込みモデルの置き場がありません: $MODEL_ROOT" >&2
  echo "[models] この配布物はモデルを同梱していません。次のどちらかで進められます。" >&2
  echo "[models]   A) 取り寄せる:  ./launch.sh --fetch-model" >&2
  echo "[models]      ※ 取り寄せ先とネットの具合によっては失敗することがあります。" >&2
  echo "[models]   B) 持っているフォルダをつなぐ: cynovela.yaml の paths: の models_dir: に" >&2
  echo "[models]      models--BAAI--bge-m3 が入っているフォルダの場所を書く" >&2
  echo "[models] 置き場の形は SETUP-ACCELERATOR.md の手順に合わせてください。" >&2
  exit 2
fi
...
#   1つの固定値だったので、受け取り手が同じ名前の入れ物を既に持っていると、文書どおりの
#   起動をしただけで確認も無く消えた (2>/dev/null || true で消したことすら見えない)。
#   置き換えるのは、この配布物が作った入れ物 (Containerfile の目印つき) だけにする。
#   目印の無い同名の入れ物が在るときは消さずに止め、別の名前で起動する方法を案内する。
...
#   そのため docker では常に「入れ物なし」と判定され、この下の保護ブロック
#   (受け取り手の入れ物を消さない・二重に起動しない・食い違いを知らせる) が丸ごと
#   素通りし、直後の run が "The container name is already in use" という生の失敗になる
#   (実測 2026-08-13)。stop.sh:17 が既に同じ非互換を認識して inspect を使っている。
#   同じ形へ揃える。実体が podman のときに走る文字列は従来と1バイトも変えていない。
_cyn_container_exists() {  # $1=入れ物の名前 → 0=在る / 1=無い
  case "$(podman --version 2>/dev/null)" in
    podman*) podman container exists "$1" 2>/dev/null ;;
    *)       podman container inspect "$1" >/dev/null 2>&1 ;;
  esac
}
if _cyn_container_exists "$NAME"; then
  ...
  if [ "$_owner" = "cynovela-container" ]; then
    # DD-CYN-0053: 同じ名前の入れ物が在るときは消さない。
    #   以前はここで podman rm -f して作り直していた。持ち主が同じでも、消せばその入れ物に
    ...
    _now_roots="$(podman inspect "$NAME" --format '{{json .Mounts}}' 2>/dev/null | "$ROOTS_PY" -c "
import json,sys
try:
...
" 2>/dev/null || true)"
    _want_roots="$(_roots list 2>/dev/null | "$ROOTS_PY" -c "
import json,sys
try:
...
" 2>/dev/null || true)"
    if [ "$_now_port" != "$HOSTPORT" ] || [ "$_now_roots" != "$_want_roots" ]; then
      echo "エラー: '$NAME' という名前の入れ物が既にありますが、いまの決めと食い違います。" >&2
      [ "$_now_port" != "$HOSTPORT" ] && echo "       ポート番号: いまの入れ物=$_now_port / 設定=$HOSTPORT" >&2
      [ "$_now_roots" != "$_want_roots" ] && echo "       取り込み元の顔ぶれが変わっています。" >&2
      echo "       束縛は入れ物を作るときにしか張れないため、そのままでは反映できません。" >&2
      echo "       消さずに止めました。反映するには、その入れ物を片づけてから、もう一度お試しください:" >&2
    ...
    echo "[hint] 開く場所と口の決めは、この入れ物を作ったときのものが続きます。"
...
#      控えの各 root を /app/ingest/<中の名前> へ :ro で子マウント。根0件なら tmpfs のみ (空)。
# DD-CYN-0032 B3/B4:
#   B3 = 根が1件も無いときは、この配布物の中のダミー資料 (dummy-corpus) を根にする (決定 9-3)。
#        場所は起動のたびに $REPO から解き直す。控えへ展開先の絶対の場所を焼き付けない。
#   B4 = 画面から根を足せるようにするため、控えそのものを入れ物へ読み書きで渡す。
#        (入れ物の中の /app/store/ingest-roots.json = ホストの $ROOTS_FILE)
#        新しく足した根が実際に読めるようになるのは、次に ./launch.sh を叩いたときである
#        (入れ物への束縛は起動時にしか張れないため)。画面にもその旨を出す。
if [ -n "$ROOTS_PY" ]; then
"$ROOTS_PY" - "$ROOTS_FILE" <<'PYEOF'
import json, os, sys
p = sys.argv[1]
...
if true; then
  ...
  # 複写する (tmpcopyup)。余分な根に見えるため複写を止める (実測 2026-07-28: 無指定だと sub1 等が出た)。
  # DD-CYN-0105 F-d: notmpcopyup は podman 固有の書き方で、docker は invalid field として拒む
  #   (実測 2026-08-12: exit 125)。docker の tmpfs は元から複写を行わないため、外しても同じ姿になる。
  #   実体が podman のときの文字列は従来と1バイトも変えていない。
  case "$(podman --version 2>/dev/null)" in
    podman*) INGEST_MOUNTS+=(--mount "type=tmpfs,destination=/app/ingest,notmpcopyup") ;;
    *)       INGEST_MOUNTS+=(--mount "type=tmpfs,destination=/app/ingest") ;;
  esac
  _root_n=0
  # DD-CYN-0107 F-c: プロセス置換 (< <(...)) は _roots の失敗を握りつぶし、控えが在るのに
  #   0件として黙ってダミー資料へ倒れていた。先に読み、失敗したら理由を出して止める。
  if [ -n "$ROOTS_PY" ]; then
    _mnt="$(_roots mount-args)" || {
      echo "エラー: 取り込み元の控え ($ROOTS_FILE) を読めませんでした。" >&2
      exit 2
    }
  else
    _mnt=""
...
# DD-CYN-0086: この Mac が使っている時間帯を、そのまま入れ物へ渡す。
#   なぜ要るか: 入れ物は自分の中に「いま何時か」を持つ。何も渡さなければ世界標準時になる。
#   画面に出る時刻には2種類あり、ブラウザが直して出すものは機材の時間になるが、
#   入れ物が自分の時計で文字にしたもの (ワークスペースの名前に焼き付く時刻など) は
...
if [ -n "$_host_tz" ]; then
  ...
  echo "[tz] この Mac の時間帯を入れ物へ渡します: ${_host_tz}"
else
  echo "[tz] この Mac の時間帯を読み出せませんでした。渡さずに起こします (入れ物の中は世界標準時になります)"
fi
...
# C-B5 20260729: 固定トークンでの自動同期は廃止した。固定の合言葉は起動形態によらず
