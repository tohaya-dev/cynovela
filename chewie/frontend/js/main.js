...
async function _fbLoad(path) {
  ...
  pathEl.textContent = _atVirtualTop
    ? lj('Ingest sources', '取り込み元')
    : _displaySourcePath(data.current_path);
  // parent_path === "" は「仮想の最上位へ戻れる」の意 (falsy 判定では上へ戻れなくなる)。
  if (upBtn) upBtn.disabled = (data.parent_path === null || data.parent_path === undefined);
  // zanken-fix4-20260706: 選択ボタンに「いま確定したら何が選ばれるか」を動的表示（親フォルダ誤選択の防止）。
  const selBtn = document.getElementById('fb-select-btn');
  if (selBtn) {
    const seg = (_displaySourcePath(data.current_path) || '').split('/').pop() || '';
    // 仮想の最上位は実パスではないため選択不可にする
    selBtn.disabled = _atVirtualTop;
    selBtn.textContent = _atVirtualTop
  ...
  if (_folders.length === 0 && _files.length === 0) {
    listEl.innerHTML = `<div style="padding:14px;color:#94a3b8;text-align:center;">${bi('(Empty)', '(空フォルダ)')}</div>`;
    return;
  }
...
function qsConfirmAndStart() {
  ...
  //   ga-close-v3 PartX: この系統は実行系の関数をもともと持たない（画面・JS ともに送信入口は無い）。
