...
function showAddSourceModal() {
  ...
  openFormModal(lj('Add folder (1/2: choose folder)', 'フォルダから追加（1/2: フォルダ選択）'), `
    ...
    <div class="form-group">
      ...
      <div style="display:flex;gap:6px;align-items:center;">
        <input id="new-src-path" class="form-input" placeholder="${escapeHtml((_ingestRootsCache && _ingestRootsCache[0] && _ingestRootsCache[0].host_path) ? lj('e.g. ', '例: ') + _ingestRootsCache[0].host_path : lj('e.g. /Users/you/Documents/myproject', '例: /Users/you/Documents/myproject'))}" style="flex:1;">
      ...
      <div class="form-hint">
        ...
        ${lj('Subfolders can each be registered as separate sources.', 'サブフォルダは、それぞれ別のソースとして登録できます。')}<br>
        ${lj('If a folder is not in the list, run ./launch.sh --add-path &lt;folder path&gt; in Terminal to register it; it becomes selectable on this screen right away.', '一覧に無いフォルダは、ターミナルで ./launch.sh --add-path &lt;フォルダのパス&gt; を実行して取り込み元に登録すると、この画面からすぐ選べるようになります。')}
      </div>
    </div>
...
function _addSourceStep2() {
  ...
  openFormModal(lj('Add folder (2/2: choose destination)', 'フォルダから追加（2/2: 追加先の選択）'), `
    <div class="form-group">
