...
FORM="host"                   # container = コンテナで動かす / host = この Mac に直接入れる
...
is_running() {
    if [ "$FORM" = "container" ]; then
        ...
        local dd pid
        dd="$(conf_get_or paths data_dir "$REPO/store")"
        case "$dd" in ./*) dd="$REPO/${dd#./}" ;; esac
        pid="$(cat "$dd/server.pid" 2>/dev/null || true)"
