...
● 番号で答えずに、はじめから決めて起動する
  ...
  ./launch.sh --remove <名前>     足したフォルダを外します (名前は --list に出るもの)。
                                  ※ 画面でも 設定 → 検索の対象フォルダ から足す・見る・外せます。
...
#   バックアップの読み口を1か所へ寄せるため、ここも scripts/ingest_roots.py で読む。
_roots_count() {
...
run_probe() {
    ...
    if [ -n "$_model_snap" ]; then
        ...
        if [ ! -t 0 ] || [ "$NO_PROMPT" = "1" ]; then
            add_warning "埋め込みモデル bge-m3 が手元にありません。この起動の仕方では確認を出せないため、起動の中でダウンロードを試みます (インターネットにつなぎます)。先に自分で置く場合は docs/operations.md の手順で $MODEL_DIR/snapshots/<版>/ へ置いてください。"
        else
            add_warning "埋め込みモデル bge-m3 が手元にありません。このまま起動すると、ダウンロードするかどうかの確認が出ます。先に自分で置く場合は docs/operations.md の手順で $MODEL_DIR/snapshots/<版>/ へ置いてください。"
        fi
    fi
...
start_app() {
    ...
    # exFAT などの外付けでは、macOS がファイルを作るたびに付属情報
