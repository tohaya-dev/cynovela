...
# DD-CYN-0117 (版7): 配布物のディレクトリツリーは、リポジトリのルートディレクトリと同じとは限らない。
...
# DD-CYN-0117 (版7): 検査値と同じく、このディレクトリツリーの下 → リポジトリのルートディレクトリ の順で探す。
...
# DD-CYN-0117 (版7): マーカーが在るとき (雛形のまま) と、既に値の2行へ置き換わって
...
# DD-CYN-0117 (版7): 検査値はこのディレクトリツリーの下 → リポジトリのルートディレクトリ の順で探す。
#   1 つのリポジトリにディレクトリツリーが複数並ぶ形では、ルートの tools/ に 1 本だけ置く運用である。
CHECK_VALUES=""
