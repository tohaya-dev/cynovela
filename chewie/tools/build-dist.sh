...
#   これが狙っていた内部参照は、外部の記録の題名の中に現れる形だった。その題名には
#   この一覧の別の検出語（外部の記録の名前そのもの）が必ず入っているので、外しても取りこぼさない。
#   🔴 ここでその語を字のまま書いてはいけない。書くと本スクリプト自身がステージに同梱されて
#      検査(c-2)が自分を検出し、パッケージングが常に止まる（実測 2026-08-29・上の(b)と同じ型）。
...
cd "$ROOT"
NAME="$(basename "$ROOT")"
# リポジトリのルートディレクトリから見た、このディレクトリツリーの位置 (ルートそのものなら空)
SUBDIR="$(git rev-parse --show-prefix)"
SUBDIR="${SUBDIR%/}"
# git archive / git ls-tree に渡すディレクトリツリー (SUBDIR が空なら $REF そのもの)
if [ -n "$SUBDIR" ]; then
  REF_TREE="$REF:$SUBDIR"
else
  REF_TREE="$REF"
...
echo "[dist] ref=$REF flavor=$FLAVOR name=$NAME root=$ROOT subdir=${SUBDIR:-(リポジトリのルートディレクトリ)}"
...
# (版7): 検査値はこのディレクトリツリーの下 → リポジトリのルートディレクトリ の順で探す。
#   1 つのリポジトリにディレクトリツリーが複数並ぶ形では、ルートの tools/ に 1 本だけ置く運用である。
CHECK_VALUES=""
...
if [ -z "$CHECK_VALUES" ]; then
  echo "[dist] 検査値 (tools/dist-check-values.local) が見つかりません。パッケージングを止めます。" >&2
  echo "[dist]   探した先: $ROOT/tools/ と $REPO/tools/" >&2
  exit 1
fi
