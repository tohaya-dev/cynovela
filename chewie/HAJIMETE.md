...
- **管理者で最初に入ると、必ず「合言葉を変えてください」と出ます。**
...
控えを扱う Python はその最初の一度で用意されます）。

**この押す道は、中身が空のデータベース（本番）で立ち上がります。** 同梱のダミー資料で試すときは、ターミナルから `./launch.sh --demo` を叩きます。
開いてすぐ、同梱の資料に質問できます。管理者・閲覧者とも、下の 2 節の合言葉でそのまま入れます。
...
最初の合言葉は、**同梱の `STARTUP.md` の「ログイン」の節**に書いてあります。

管理者で入ると合言葉の変更を求められます。**ここで必ず変えてください。**
...
```
読み込み中 → チャンク書き込み中 → 伏字処理中 → 伏字処理中(まとめ) → Embedding生成中 → 完了
```

**画面を閉じても取り込みは続きます。** 開き直すと、いまの段と何件目かに戻ります。
大きな資料では伏字の段に時間がかかりますが、件数が動き続けていれば進んでいます。
...
| 画面に出ること | どうするか |
|---|---|
| 「取り込み元がまだ1件もありません」 | 画面の **「取り込み元を足す」** を押す（3節）。この案内の下にその道へ行くボタンが出ます |
| 「初回パスワードの変更が必要です」 | 0節。先に合言葉を変える |
| 「ポート 8765 を別のものが使っています」 | 別の番号で上げる: `./launch.sh --port <別の番号>` |
| 「足りないものがあるので起動しません」 | `./launch.sh --setup` を実行する |
| 進み具合が止まって見える | 件数が動いていれば進んでいます。伏字の段は時間がかかります |
