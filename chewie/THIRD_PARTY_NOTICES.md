...
## Python packages in the runtime environment (.venv-cynovela) / Python 実行環境 (.venv-cynovela) の部品
...
「いまダウンロードする」を選んだ場合に、同じ入手元（Hugging Face）から同じモデルを
ダウンロードします。ライセンスの扱いは同梱する場合と同じです。
