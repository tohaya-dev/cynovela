...
#   直す必要がある。二度走らせるのは安全でないため、済んだことを示すファイルを環境の中に残す。
...
_drop_marks() {
    ...
    #   1 行の警告を出す。削除する対象は「配布物に付いた拡張属性」であって、
    #   読み取り専用で配られたバンドルには元から付いていない。
    ...
    if [ -n "$_err" ]; then
        echo "注意: 次の拡張属性は削除できませんでした。そのまま起動を続けます:"
        printf '%s\n' "$_err" | sed 's/^/    /'
    fi
}
...
_warn_if_cloud_synced() {
    ...
    echo "     - 同期がファイルの実体を退避すると、._ ファイルの削除・拡張属性の処理・"
...
if [ "$_PASSTHRU" = "1" ] || [ ! -t 0 ] || [ ! -t 1 ]; then
    # --follow / --pro は包みだけが読む指定であり、本体は知らない。
    # 透過の道では包みの後段 (これらを読む所) に届かないため、素通りさせると
    # 本体が「知らない指定です」で落ち、理由が伝わらない。ここで断る。
    for _a in "$@"; do
        case "$_a" in
            --follow|--pro)
                echo "この指定 ($_a) は、端末を繋いだ呼び出しでだけ使えます。この呼び出しでは対応していません。" >&2
                exit 2
                ;;
        esac
    done
