...
split files** (`cynovela-chewie-models-1.1.1.tar.gz.part00`–`part02`). The
...
`tar -xzf ../cynovela-chewie-models-1.1.1.tar.gz` inside the unpacked chewie
...
All forms are version `1.1.1`. Where the forms differ, this document says so.
...
配布物には**パッケージ版**（1本・約800MB。Python も conda も不要で、展開して `./launch.sh` だけで動く形）・**全部入り**（ソース版・モデル込み・分割3本）・**モデル別取得版**（ソース版・軽量・1本数MB）の 3 つの形があり、これに **AIモデルだけの分割ファイル（models）** が加わります。パッケージ版とモデル別取得版には AIモデルが入っていないので、models も落として重ねます。形によって話が違うところは、そのつど断ります。版はいずれも `1.1.1` です。
