...
版は `1.0.1` です（`core/version.py` の `APP_VERSION` が唯一の出どころで、
...
- **画像は既定では本文を読みません。** 既定の動作はファイル名だけを索引に入れる
...
- 旧来の経路 `/api/transcribe` は**もう繋がっていません**。
  `server.py` で `include_router` を外してあります（伏字前の文字起こしがそのまま
  返る穴だったため、あえて外したものです）。
...
- **伏字が失敗したら止まる。** 質問文・回答のいずれも、
  ...
  伏字前のものを代わりに返すことはしません（`routers/chat.py`）。
- **伏字なし取り込みと外部埋め込みの組み合わせは拒否します。**
  古い版で作られた「伏字なし」のコレクションは、外部の埋め込みを有効にしている間は
...
- ベクター索引（ChromaDB）も同じマシンのファイルです。
  複数のプロセスから同じ索引を書き換えないでください。
...
- **埋め込みモデルは、版（snapshot）まで一致していなければなりません。**
  ...
  **版が違うとベクトルの数値が変わり、同梱済みの索引と混ざって検索順位が壊れます。**
...
- **コンテナ版の配布物には `deploy/k8s/20-deployment.yaml` が入っていません。**
  配布物を作る `tools/build-dist.sh` が、梱包の直前にこのファイルを落としています。
...
- 同梱の索引とデータベースは、配布物を作るときに**配布物の中の `dummy-corpus/` だけ**から
