...
This package is the **application build (runs on macOS directly)**.
...
3. **Add what it should read.** Double-click `Cynovela-add-folder.command` and
   pick a folder.
4. **Ask a question.** Open `http://localhost:8765` and type in plain
...
3. **読ませるものを足す。** `Cynovela-add-folder.command` をダブルクリックして
   フォルダを選びます。
4. **質問する。** `http://localhost:8765` を開き、普通の言葉で聞きます。
